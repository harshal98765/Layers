...
export default function InfiniteMenu({ items = [] }) {
  ...
  const infiniteGridMenuRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (!infiniteGridMenuRef.current) {
      infiniteGridMenuRef.current = new InfiniteGridMenu(
        canvas,
        ...
        setIsMoving,
        (sk) => {  // This is the onInit callback
    ...
    const handleResize = () => {
      if (infiniteGridMenuRef.current) {
        infiniteGridMenuRef.current.resize();
      }
    };
